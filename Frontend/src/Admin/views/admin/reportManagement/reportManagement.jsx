...
function ReportManagement() {
  ...
  async function fetchTopSpots() {
    const response = await fetch(
      "http://localhost:5000/api/get-most-booked-spots"
    );
    const data = await response.json();
  ...
  async function fetchTopListers() {
    const response = await fetch(
      "http://localhost:5000/api/get-most-booked-listers"
    );
    const data = await response.json();
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 7; i++) {
      daywiseOrdersObjArrFinal = [
        ...daywiseOrdersObjArrFinal,
        daywiseOrdersObjArr[dayNum.indexOf(i)],
      ];
    }
  ...
  return (
    <div className="flex-row grid grid-cols-1">
      ...
      <div className="my-8 grid grid-cols-2 gap-5">
        <CheckTable
          title="Best Selling Spots"
          columnsData={columnsDataSpots}
          tableData={bestSpots}
        />
        <CheckTable
          title="Top Spots Listers"
          columnsData={columnsDataListers}
          tableData={bestListers}
        />
        <DailyTraffic />
        <PieChartCard />
      </div>
