...
function AmenitiesManagement() {
  ...
  const handleAddAmenity = useCallback(async (name, icon) => {
    ...
    const res = await axios.post(
      `http://192.168.1.104:5000/api/update-amenities`,
      form
    );
    const resData = res.data;
    if (resData.success === true) {
  ...
  const handleAddCategory = useCallback(async (name, icon) => {
    ...
    const res = await axios.post(
      `http://192.168.1.104:5000/api/update-category`,
      form
    );
    const resData = res.data;
    if (resData.success === true) {
  ...
  const deleteCategory = async (id) => {
    try {
      const res = await axios.delete(
        `http://192.168.1.104:5000/api/delete-category/${id}`
      );
      const resData = res.data;
      if (resData.success === true) {
  ...
  const deleteAmenity = async (id) => {
    try {
      const res = await axios.delete(
        `http://192.168.1.104:5000/api/delete-amenities/${id}`
      );
      const resData = res.data;
      if (resData.success === true) {
