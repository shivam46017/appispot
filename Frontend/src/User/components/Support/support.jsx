...
function SupportForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`http://localhost:5000/api/support/${localStorage.getItem('userId')}?role=${localStorage.getItem('userRole')}`, form)
  ...
  return (
    <div className="min-h-screen grid grid-cols-2 p-28">
      ...
      <div className="flex flex-col justify-start">
        <span className="font-bold text-xl">Something</span
        >
      </div>
      <form onSubmit={handleSubmit} className="w-full p-4">
        ...
        <div className="mb-3">
          ...
          <input
            ...
            required
            accept="image/*"
            onChange={(e) => dispatchScreenshot({ type: 'ADD-ARRAY-OF-SCREENSHOTS', screenshots: e.target.files })}
            onDrag={(e) => dispatchScreenshot({ type: 'ADD-ARRAY-OF-SCREENSHOTS', screenshots: e.target.files })}
          />
        </div>
        <div className="mb-3 flex gap-3">
          {
            preview.map((value, i) => {
              return (
                <Chip key={`preview-${i}`} sx={{ height: '100%', padding: 1 }} label={<img width={120} className="rounded-xl" src={preview[i]} />} onDelete={() => {
                  dispatchPreview({ type: 'REMOVE-PREVIEW', removeFromIndex: i })
                  dispatchScreenshot({ type: 'REMOVE-SCREENSHOT', removeIndex: i })
                }} />
              )
            })
          }
