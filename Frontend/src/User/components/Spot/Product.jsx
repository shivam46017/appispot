...
export default function Spot() {
  ...
  async function discount() {
    ...
    const response = await axios.post(
      "https://many-aerial-innovation-programming.trycloudflare.com/api/discountvenue",
      {
        venueCategories: cat,
  ...
  async function sendMessage() {
    const response = await fetch(
      chats?.length == 0
        ? "https://many-aerial-innovation-programming.trycloudflare.com/api/conversation/add"
        : "https://many-aerial-innovation-programming.trycloudflare.com/api/message/add",
      {
        method: "POST",
        body: JSON.stringify(
          chats?.length == 0
  ...
  async function getAllMessages() {
    const response = await fetch(
      `https://many-aerial-innovation-programming.trycloudflare.com/api/conversation/getAll?senderId=${localStorage.getItem(
        "userId"
      )}&receiverId=${spotDetails?.lister}`
    );
  ...
  useEffect(() => {
    ...
    async function getSpotDetails() {
      const response = await fetch(
        `https://many-aerial-innovation-programming.trycloudflare.com/api/getspot/${params.spotId}`
      );
      const data = await response.json();
              ...
              spotDetails?.Images.map((item, index) => {
                return (
                  ...
                  key={`spot-details-image-${index}`}
                    src={`https://many-aerial-innovation-programming.trycloudflare.com${
