...
function Messages() {
  ...
  const [message, setMessage] = useState("")
    ...
    async function sendMessage () {
        const response = await fetch(myChat?.length == 0 ? "http://192.168.1.104:5000/api/conversation/add" : "http://192.168.1.104:5000/api/message/add", {
            method: "POST",
    ...
    async function getAllMessages () {
      console.log("Getting messages")
        const response = await fetch(`http://192.168.1.104:5000/api/conversation/getAll?receiverId=${localStorage.getItem("userId")}`)
