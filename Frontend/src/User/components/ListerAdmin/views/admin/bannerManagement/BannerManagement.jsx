...
function Messages() {
  ...
  const [message, setMessage] = useState("")
    ...
    async function sendMessage () {
        const response = await fetch(myChat?.length == 0 ? "/api/conversation/add" : "/api/message/add", {
            method: "POST",
    ...
    async function getAllMessages () {
      console.log("Getting messages")
        const response = await fetch(`/api/conversation/getAll?receiverId=${localStorage.getItem("userId")}`)
